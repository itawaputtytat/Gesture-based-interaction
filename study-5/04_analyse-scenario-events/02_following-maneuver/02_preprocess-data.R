...
sett_proc$interpolate <- F # Only necessary for time headway
sett_proc$plot <- F
...
## Load condition data from data base
## ... and reshape information
dat_conditions <- 
...
## Create condition data for baseline
...
## Join condition data and condition rows for baseline
dat_conditions <- 
  rbind(dat_conditions,
        dat_conditions_baseline)

## Join data and condition data
assign( sett_dat$df_name,
...
## Workaround for NA in is_usable (strange joining behavior)
assign(sett_dat$df_name,
       get(sett_dat$df_name) %>% 
             filter(!is.na(is_usable)))



# Interpolation -----------------------------------------------------------

if (sett_proc$interpolate) {
  
  intrpldf_batch(sett_dat$df_name,
                 #col_name_ref = sett_proc$col_name_am,
                 col_name_ref = "sxx_exx_tti_s",

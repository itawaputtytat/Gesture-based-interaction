...
dat2plot.wide2 <- 
  ...
  summarise(glances_n = mean(glances_n))

dat2plot.wide3 <- 
  spread(dat2plot.wide2, condition, glances_n) %>% 
  group_by(id) %>% 
  summarise(city_gestures_complex = max(gsk, na.rm = T),
            city_gestures_simple = max(gse, na.rm = T),
            city_touch_complex = max(tsk, na.rm = T),
            city_touch_simple = max(tse, na.rm = T),
            motorway_gestures_complex = max(gak, na.rm = T),
            motorway_gestures_simple = max(gae, na.rm = T),
            motorway_touch_complex = max(tak, na.rm = T),
            motorway_touch_simple = max(tae, na.rm = T))

dat2plot.wide4 <- dat2plot.wide3
dat2plot.wide4[is.na(dat2plot.wide4)] <- 0 

write.table(dat2plot.wide4, "clipboard", sep="\t", row.names = FALSE)
...
dat2plot2 <- 
  dat2plot %>% 
  select(id, itype, ilevel, scenario,
         glances_n) %>% 
  mutate(itype = as.factor(itype),
         ilevel = as.factor(ilevel),
         scenario = as.factor(scenario))

dat2plot3 <- 
  expand.grid(
    id = unique(dat2plot2$id),
    itype = unique(dat2plot2$itype),
    ilevel = unique(dat2plot2$ilevel),
    scenario = unique(dat2plot2$scenario))
#dat2plot3$glances_n <- 0

dat2plot4 <- 
  left_join(dat2plot3,
            dat2plot2,
            by = c("id", "itype", "ilevel", "scenario"))

dat2plot4$glances_n[is.na(dat2plot4$glances_n)] <- 0

model_glances_n <- 
  ezANOVA(data = dat2plot2, 
          dv = .(glances_n), 
          wid = .(id), 
